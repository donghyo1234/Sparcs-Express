...
const AccountPage = () => {
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { credential: {id : SID, pw : SPW} });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { credential: {id : SID, pw : SPW}, amount: amount });
      setNTransaction(0);
      if (!data.success) {
